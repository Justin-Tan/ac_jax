...
class Evaluator:
    ...
    def analyse(self, sample: str, island_id: int | None, version_generated: int | None) -> None:
        ...
        for current_input in self._inputs:
            test_output, complete_flag = self._sandbox.run(
                program, self._function_to_run, current_input, self._timeout_seconds)
            if (complete_flag and not code_utils.calls_ancestor(program, self._function_to_evolve) and test_output is not None):
